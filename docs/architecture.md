...
| `documind/documents/` | `DocumentLoader` — the one way a file becomes a `Document`; `PdfLoader` (per-page text, OCR fallback) and `TextLoader` (`.txt`/`.md`) implement it; `LoaderFactory` picks between them by extension | pdfplumber, `ocr.py` |
